...
if (!usuarioId) {
  ...
  return;
    ...
    try {
      ...
      alert(error.response?.data?.mensagem || "Erro ao salvar tarefa. Tente novamente.");
    }
  };
                ...
                {tarefas.length > 0 ? (
                  tarefas.map((tarefa) => (
                    <tr key={tarefa.idTarefa}>
                      ...
                      <td>
                        <span className={`badge ${tarefa.prioridade === 'Alta' ? 'bg-danger' : tarefa.prioridade === 'Média' ? 'bg-primary' : 'bg-secondary'}`}>
                          {tarefa.prioridade}
                        </span>
                      </td>
                      <td>{tarefa.prazo?.slice(0, 10) || '---'}</td>
                      <td className="action-buttons">
                        <button className="btn btn-primary" onClick={() => handleEdit(tarefa)}>
                          <i className="bi bi-pencil-fill"></i>
                        </button>
                        <button className="btn btn-danger" onClick={() => handleDelete(tarefa.idTarefa)}>
